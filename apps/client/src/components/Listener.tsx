...
export default function Listener({ symbol, viewSymbol }: { symbol: string, viewSymbol: string }) {
  ...
  }, [container.current, viewSymbol, theme]);
            ...
            <TabsContent value="logs">
              <ScrollArea className="rounded-md border h-[200px]">
                {messages.map((message: string, index: number) => (
                  <p key={index} className="px-3.5">{message}</p>
                ))}
              </ScrollArea>
            </TabsContent>
